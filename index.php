...
{"name": "被分享清單","page": false},{"name": "被分享項目","page": false},{"name": "分享清單","page": false} ];
...
if(Setview=='group'){
  ...
  {"name": "被分享清單","page": true},{"name": "被分享項目","page": false},{"name": "分享清單","page": false} ];
}
else if(Setview=='besitem'){//被分享清單頁面動作後回傳狀態
  var json_baseDate = [{"name": "類別","page": false},{"name": "清單","page": false},{"name": "項目","page": false}, 
  {"name": "被分享清單","page": false},{"name": "被分享項目","page": true},{"name": "分享清單","page": false} ];
}
else if(Setview=='slist'){//分享清單頁面動作後回傳狀態
  var json_baseDate = [{"name": "類別","page": false},{"name": "清單","page": false},{"name": "項目","page": false}, 
  {"name": "被分享清單","page": false},{"name": "被分享項目","page": false},{"name": "分享清單","page": true} ];
}
       ...
       <div class="page" v-if="list[1].page==true">
        ...
        <div class="content">
         ...
         <div class="page" v-if="list[2].page==true">
          <div class="title"><?php echo $gname."類-"?>【<?php echo $lname?>】清單</div>
          <div class="content">
          ...
          <div class="page" v-if="list[3].page==true">
            <div class="title">【<?php echo $uno ?>】{{list[3].name}}</div>
            <div class="content">
              <div class="list">
                <ul>
                  <?php
                  try {
                    ...
                    foreach ($stid as $row) {
                      ...
                      <li>
                        <div class="name">
                          <a href="index.php ?SNO=<?php echo $row["SNO"]?>&
                            GNO=<?php  echo $row["GNO"]?>&
                            LNO=<?php  echo $row["LNO"]?>&
                            GNAME=<?php echo $gname?>&
                            LNAME=<?php echo $row["TITLE"]?>&
                            ...
                            >
                            <?php echo $row["TITLE"]?>
                            <FONT>擁有者ID：【<?php echo $row["SNO"];?></FONT>】
                          </a></div>
                          <div class="tool">
                           <a href="Main.php?SNO=<?php echo $row["SNO"]?>&
                             GNO=<?php echo $row["GNO"];?>&
                             LNO=<?php echo $row["LNO"];?>&
                             UNO=<?php echo $uno;?>&
                             set=BES_Cancel"
            ...
            <div class="page" v-if="list[4].page==true">
              <div class="title">被分享清單【<?php echo $_SESSION['LNAME']?>】</div>
              <div class="content">
                <div class="name" style="text-align:center;"><font size ="4"face="DFKai-sb">被分享項目</font></div>
                <div class="tool">
                  <!--I_CREATE-->
                  <form action="Main.php" method="get">
                    <input type="hidden" name="UNO" value="<?php echo $uno?>">
                    ...
                    <input type="hidden" name="LNO" value="<?php echo $lno?>">
                    <input type="hidden" name="GNAME" value="<?php echo $uname?>">
                    <input type="hidden" name="set" value="BES_I_CREATE">
                    <input type="text"   name="CONTENT" placeholder="新增項目內容">
                    <input type="submit" value="新增">
                  </form>
                </div>
                <div class="list">
                  <ul>
                    ...
                    try {
                      $conn = _dbconnect();
                      $stid=_BES_selectitem($_SESSION['SNO'],$_SESSION['GNO'],$_SESSION['LNO'],$_SESSION['UNO']);   
                      ...
                      foreach ($stid as $row) {
                        ...
                        <li> 
                          ...
                          <div class="tool">
                            ...
                            <!--I_EDIT-->
                            <form action="Main.php" method="get">
                              <input type="hidden" name="UNO" value="<?php echo $row["SNO"]?>">
                              <input type="hidden" name="GNO" value="<?php echo $row["GNO"]?>">
                              <input type="hidden" name="LNO" value="<?php echo $row["LNO"]?>">
                              <input type="hidden" name="INO" value="<?php echo $row["INO"]?>">
                              <input type="hidden" name="set" value="BES_I_EDIT">
                              ...
                              if(strlen($row["ONCHECK"])!="")
                              {
                                echo '<a href="Main.php ?SNO='.$row["SNO"].'&';
                                echo  'GNO='. $row["GNO"].'&';
                                echo  'LNO='. $row["LNO"].'&';
                                echo  'INO='.$row["INO"].'&';
                                echo  'UNO='.$_SESSION["UNO"].'&';
                                echo  'set=BES_I_UNCHK">已確認</a>&nbsp;';
                              }else{
                                echo '<a href="Main.php ?SNO='.$row["SNO"].'&';
                                echo  'GNO='. $row["GNO"].'&';
                                echo  'LNO='. $row["LNO"].'&';
                            ...
                            <!--I_DEL-->
                            <a href="Main.php ?UNO=<?php  echo $row["SNO"]?>&
                              GNO=<?php  echo  $row["GNO"]?>&
                              LNO=<?php  echo  $row["LNO"]?>&
                              INO=<?php  echo $row["INO"]?>&
                              set=BES_I_DEL"class="delete">刪除</a>
                            </div>
                          </li>
                          <?php 
                        }
              ...
              <!--分享之清單-->
              <div class="page" v-if="list[5].page==true">
                <div class="title">【<?php echo $uname ?>】的{{list[5].name}}</div>
                <div class="content">
                  <div class="list">
                    <ul>
                      <?php
                        ...
                        foreach ($stid as $row) {
                          ...
                          <li>
                            <div class="name"><a href="http://">
                              <?php echo $row["TITLE"];?>清單 | 分享至ID:【<?php echo $row["UNO"];?>】 
                            </a></div>
                            <div class="tool">                    
                              <a href="Main.php? UNO=<?php echo($uno)?>&
                                GNO=<?php echo $row["GNO"];?>&
